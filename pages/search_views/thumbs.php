...
<div class="ResourcePanelShell" id="ResourceShell<?php echo htmlspecialchars($ref)?>">
	...
	<table border="0" class="ResourceAlign<?php if(!hook("replaceresourcetypeicon")){ if (in_array($result[$n]["resource_type"],$videotypes)) { ?> IconVideo<?php } ?><?php } hook('searchdecorateresourcetableclass'); ?>">
...
<?php } ?> <!-- END HOOK Rendertitlethumb -->			
		...
		for ($x=0;$x<count($df);$x++)
			...
			<?php if (!hook("replaceresourcepanelinfonormal")){?>
			...
			<?php } ?>
			<?php } /* end hook replaceresourcepanelinfonormal */?>
